...
public:
      ...
      : io_context_(io_context), socket_(io_context)
  {
    ...
    {
      char line[chat_message::max_body_length + 1] = ":setusername:";

      strncat(line, login.c_str(), chat_message::max_body_length + 1);
    ...
    {
      char line[chat_message::max_body_length + 1] = ":setpassword:";

      strncat(line, password.c_str(), chat_message::max_body_length + 1);
...
int main(int argc, char* argv[])
{
  try
  {
    ...
    char suff[chat_message::max_body_length + 1 - c.login.length() - 3];
    while (cin.getline(suff, chat_message::max_body_length + 1 - c.login.length() - 3))
    {
      using namespace std; // For strlen and memcpy.
      strcpy(line, c.login.c_str());
      strcat(line, " : ");
      strcat(line, suff);
      chat_message msg;
      msg.body_length(strlen(line));
